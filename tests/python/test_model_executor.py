...
@pytest.mark.parametrize("tp,ep,moe_tp", [(1, 1, 1), (16, 16, 1), (16, 1, 16)])
def test_executor_accepts_prepared_glm_tensor_and_expert_parallel(tp: int, ep: int, moe_tp: int) -> None:
    backend = _PreparedStubAttentionBackend()
    config = {
...
def test_executor_rejects_prepared_glm_split_topologies(split: str) -> None:
    with patch(
        ...
        executor = ModelExecutor(_FakeModel(), {"model_type": "glm_moe_dsa", split: 2}, max_seqs_per_batch=2)
    assert not executor.supports_prepared_metadata
...
def test_prepared_executor_rejects_mtp_state_before_model_execution() -> None:
    ...
    executor.decode_graph_runner.execute_prepared.assert_not_called()
